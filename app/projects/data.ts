...
export const projects: Project[] = [
  ...
  },
  {
    ...
    contributions: [
      {
        title: "유저·마이페이지 API",
        tech: ["Spring Boot", "Spring Security", "Cloudinary"],
        points: [
    ...
    contributions: [
      {
        ...
        points: [
          "상품 주문 후 가상계좌를 발급하고, 입금 완료 보고·판매자 승인·만료 처리로 이어지는 결제 API를 구현했습니다.",
          "결제 상태를 발급, 입금대기, 승인, 만료 단계로 분리하고, 각 API가 허용된 상태에서만 다음 단계로 넘어가도록 검증 로직을 연결했습니다.",
          "판매자가 본인 상품의 결제만 승인할 수 있도록 권한 검증을 추가하고, 승인 시 계좌 마스킹과 결제 이력 저장까지 이어지도록 처리했습니다.",
        ],
      },
      {
        title: "입금 검증 시뮬레이터",
        ...
        points: [
          "실제 결제 상태를 변경하지 않고 입금 요청의 유효성만 확인하는 시뮬레이션 API를 구현했습니다.",
          "요청자 이메일과 계좌 소유자 일치 여부, 계좌 상태, 주문 금액 일치 여부를 순차적으로 검증했습니다.",
          "이메일 불일치는 권한 오류로 분리하고, 금액·상태 불일치는 실패 응답으로 내려보내 성공/실패 케이스를 명확히 구분했습니다.",
        ],
      },
      {
